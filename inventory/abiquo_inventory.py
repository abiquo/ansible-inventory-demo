...
Requires some python libraries, ensure to have them installed when using this script. You can install
...
`vdc_virtualdatecenter`: Creates a host group for each virtualdatacenter containing all hosts defined on it
`vapp_virtualappliance`: Creates a host group for each virtualappliance containing all hosts defined on it
`vdc_virtualdatecenter_vapp_virtualappliance`: Creates a host group for each virtualdatacenter/virtualappliance 
pair containing all hosts defined on it
`template_TEMPLATENAME`: Creates a host group for each image template containing all hosts using it
`hwprof_HWPROFILE`: Creates a host group for each hardware profile containing all hosts using it
`var_VARNAME_VARVALUE`: Creates a host group for each variable/value pair. It will contain all the hosts
